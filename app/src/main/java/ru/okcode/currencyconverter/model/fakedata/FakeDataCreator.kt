...
class FakeDataCreator {
    companion object {
        fun createFakeRatesData(): Result<RatesData> {
            val rate1 = Rate("RUB", R.drawable.ic_rub,R.string.RUB, 80.5, 80.5)
            val rate2 = Rate("EUR", R.drawable.ic_eur,R.string.EUR, 1.0, 1.0)
            val rate3 = Rate("USD", R.drawable.ic_usd,R.string.USD, 1.02, 1.02)
            val ratesList: List<Rate> = listOf(rate1, rate2, rate3)
            val date = Date()
            val baseCurrency = "EUR"
            val fakeRatesData = RatesData(date, baseCurrency, ratesList)
